#!/usr/bin/env bash
# top_users_by_runtime.sh — Find top N users by total job runtime from Slurm jobcomp log filtered by X days
...
echo "📊 Showing top $TOP_N users by total runtime (seconds) from $LOG_FILE"
...
RESULT=$(awk -v cutoff="$cutoff" -v days="$DAYS" '
  {
    ...
    for (i=1; i<=NF; i++) {
      if ($i ~ /^StartTime=/) {
        split($i, a, "="); gsub("T", " ", a[2]); start=a[2];
      }
      if ($i ~ /^EndTime=/) {
        split($i, b, "="); gsub("T", " ", b[2]); end=b[2];
      }
      if ($i ~ /^UserId=/) {
    ...
    if (user != "" && start != "" && end != "") {
      st = mktime(gensub(/[-:]/, " ", "g", start));
      et = mktime(gensub(/[-:]/, " ", "g", end));
      runtime = et - st;

      if (runtime > 0 && (days == 0 || st >= cutoff)) {
        total[user] += runtime;
      }
    }
  }
